nu_comm_server <- function(input, output, session) {
  ...
  output$time_income <- renderPlot({
    ...
    income <- ia_counties_temporal_tidy %>%
      ...
      filter(!(race_ethnicity %in% c("Some Other Race Alone", "Two Or More Races", 
                                     "White Alone", "Native Hawaiian And Other Pacific Islander Alone", 
  ...
  output$time_homeownership <- renderPlot({
    ...
    homeownership <- ia_counties_temporal_tidy %>%
      filter(county_name %in% input$region_choice2 & substr(variable, 1, 6) == "B25003") %>%
      separate(concept, into = c("concept", "race_ethnicity"), sep = "\\(", remove = F) %>%
      mutate(race_ethnicity = str_trim(str_to_title(str_remove_all(race_ethnicity, "HOUSEHOLDER|\\)"))),
  ...
  employment <- reactive({
    ...
    employment <- ia_counties_temporal_tidy %>%
      filter(county_name %in% input$region_choice3 & substr(variable, 1, 6) == "B20005" & variable_index %in% c("002", "003", "049", "050")) %>%
  ...
  output$time_employment_female <- renderPlot({
  ...
  output$time_employment_male <- renderPlot({
